...
def _java_wrap_cc_impl(ctx):
    ...
    for target in ctx.attr.deps:
        ...
        #for header in cc_context.direct_headers:
        #print(header)

        #for path in cc_context.external_includes:
        #print(path)

        #for path in cc_context.framework_includes.to_list():
        #print(path)

        #for path in cc_context.quote_includes.to_list():
        #print(path)

        #for path in cc_context.system_includes.to_list():
        #print(path)
    ...
    all_include_paths = ""
    for path in depset(transitive = include_path_sets).to_list():
        all_include_paths += path
        all_include_paths += " "

    #print(all_include_paths)
