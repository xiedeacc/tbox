  ...
  for (struct ifaddrs* ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    try {
      ...
      folly::IPAddress ip_addr(
          folly::IPAddressV4::fromLong(ntohl(sa->sin_addr.s_addr)));
      ...
      if (unique_addresses.insert(ip_str).second) {
        LOG(INFO) << "Detected IPv4 address: " << ip_str << " on interface: "
...
std::vector<std::string> Util::GetLocalIPv6Addresses() {
  ...
  for (struct ifaddrs* ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    try {
      ...
      if (unique_addresses.insert(ip_str).second) {
        LOG(INFO) << "Detected IPv6 address: " << ip_str << " on interface: "
                  << (ifa->ifa_name ? ifa->ifa_name : "unknown");
      }
    } catch (const std::exception& e) {
...
std::vector<std::string> Util::GetPublicIPv6Addresses() {
  ...
  for (struct ifaddrs* ifa = ifaddrs_ptr; ifa != nullptr; ifa = ifa->ifa_next) {
    ...
    try {
      ...
      std::string ip_str = ipv6_addr.str();
      if (unique_addresses.insert(ip_str).second) {
        LOG(INFO) << "Detected public IPv6 address: " << ip_str
                  << " on interface: "
                  << (ifa->ifa_name ? ifa->ifa_name : "unknown");
      }
    } catch (const std::exception& e) {
