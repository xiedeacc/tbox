...
WebSocketClient::WebSocketClient(const std::string& host,
    ...
    : host_(host),
      ...
      ioc_(),
      ws_(ioc_),
      message_queue_(1024),  // Queue size of 1024 messages
      running_(false),
...
void WebSocketClient::Stop() {
  ...
  running_ = false;
  LOG(INFO) << "Message queue closing";
...
void WebSocketClient::ProcessMessages() {
  ...
  while (running_) {
    try {
      std::string msg;
      if (message_queue_.readIfNotEmpty(msg)) {
        HandleMessage(msg);
      } else {
...
void WebSocketClient::ReadMessage() {
  ws_.async_read(read_buffer_, [this](boost::beast::error_code ec,
    ...
    try {
      // Get the message payload (Boost.Beast has already
      // handled the WebSocket protocol)
      std::string msg = boost::beast::buffers_to_string(read_buffer_.data());
      LOG(INFO) << "Received message of size: " << msg.size();
      message_queue_.blockingWrite(std::move(msg));
