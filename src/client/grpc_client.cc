...
bool GrpcClient::Init() {
  ...
  uint32_t report_interval_seconds = config_manager->ReportIntervalSeconds();
  int login_retry_seconds = 60;
  if (report_manager->Init(channel_, stub_, report_interval_seconds,
                           login_retry_seconds)) {
  ...
  auto ddns_manager = impl::DDNSManager::Instance();
  if (ddns_manager->Init()) {
    LOG(INFO) << "DDNS manager initialized";
  } else {
    LOG(ERROR) << "Failed to initialize DDNS manager";
  }
...
bool GrpcClient::IsRunning() const {
  auto report_manager = ReportManager::Instance();
  auto ddns_manager = impl::DDNSManager::Instance();
  return report_manager->IsRunning() || ddns_manager->IsRunning();
}
