...
using grpc::Status;
using tbox::proto::MEGAService;
using tbox::proto::EC2InstanceRequest;
using tbox::proto::EC2InstanceResponse;
using tbox::proto::OpCode;
 ...
 public:
  ...
  bool ManageInstance(const std::string& instance_id, 
    ...
    if (status.ok()) {
      ...
      return response.err_code() == tbox::proto::ErrCode::SUCCESS;
    } else {
      std::string operation_name = (operation == OpCode::OP_EC2_START) ? "Start" : "Stop";
